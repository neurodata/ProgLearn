...
def generate_spirals(
    ...
    if n_class == 2:
        ...
        raise ValueError(
                    "sorry, can't currently surpport %s classes " %n_class
                )
    ...
    if n_class == 2:
        ...
        t = np.linspace(
            0,  
            np.pi* 4 * turns/n_class, 
            int(n_samples/n_class)) + np.random.normal(
                0, noise, int(n_samples/n_class
                )
            )
        dx = r * np.cos(t)
        dy = r * np.sin(t)
