...
def experiment(n_task1, n_task2, n_test=1000, 
    ...
    KNN_class = KNNSimpleClassificationDecider #HERE
    ...
    if n_task1 == 0:
        ...
        progressive_learner.add_task(X_task1, y_task1,
                                     num_transformers=n_trees, transformer_voter_decider_split=[0.33, 0.33, 0.33])
...
def experiment2(n_task1, n_task2, n_test=1000, 
               task1_angle=0, task2_angle=np.pi/2, 
               n_trees=10, max_depth=None, random_state=None):
    ...
    KNN_class = KNNClassificationDecider #HERE
    KNN_decider_kwargs = {"classes" : np.arange(2), "k" : 1}
...
def bte_v_angle1(angle_sweep,task1_sample,task2_sample,mc_rep):
...
def bte_v_angle2(angle_sweep,task1_sample,task2_sample,mc_rep):
    mean_te = np.zeros(len(angle_sweep), dtype=float)
    for ii,angle in enumerate(angle_sweep):
        error = np.array(
            Parallel(n_jobs=-1,verbose=0)(
