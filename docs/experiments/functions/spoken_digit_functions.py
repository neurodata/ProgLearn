...
from tensorflow.keras.backend import clear_session  # To avoid OOM error
...
def display_spectrogram(x_spec_mini, y_number, y_speaker, num):
    ...
    fig, axes = plt.subplots(
        nrows=6, ncols=8, sharex=True, sharey=True, figsize=(12, 9)
    )
    for j, speaker in enumerate(label_speaker):
        opt = np.where((y_speaker == speaker) & (y_number == num))[0]
        ch = np.random.choice(
            opt, 8, replace=False
        )  # randomly choose 8 different samples from the speaker to display
    ...
    plt.subplots_adjust(
        top=0.92, bottom=0.08, left=0.10, right=0.95, hspace=0.05, wspace=0.05
    )
    plt.suptitle(
        "Short-Time Fourier Transform Spectrogram of Number " + str(num), fontsize=18
    )
...
def plot_results(acc, bte, fte, te, model):
    ...
    for i in range(num_tasks):
        ...
        ax[0][1].plot(ns, et, c=clr, linewidth=2.6)
    ...
    for i in range(num_tasks):
        et = np.asarray(te[i])
        ns = np.arange(i + 1, num_tasks + 1)
        ax[1][0].plot(ns, et, c=clr, linewidth=2.6)
    ...
    for i in range(num_tasks):
        ...
        ns = np.arange(i + 1, num_tasks + 1)
        ax[1][1].plot(ns, et, c=clr, linewidth=2.6)
...
def plot_paper_figure(bte, fte):
    ...
    log_lbl = np.round(np.log([1, 1.35, 1.7]), 2)
