...
def fte_bte_experiment(
    ...
    for task_num in range((which_task - 1), 10):
        ...
        if task_num == (which_task - 1):
            if model == "synn":
                learner.add_task(
                    X=train_x[(task_num * 300) : ((task_num + 1) * 300)],
                    y=train_y[(task_num * 300) : ((task_num + 1) * 300)],
                    task_id=task_num,
                    num_transformers=1,
                    transformer_voter_decider_split=[0.67, 0.33, 0],
                    decider_kwargs={
                        "classes": np.unique(
                            train_y[(task_num * 300) : ((task_num + 1) * 300)]
                        )
                    },
                )
            elif model == "synf":
            ...
            t_num = 0
            # Add tasks for all task up to current task (task t)
            while t_num < task_num:
                # Make a prediction on task t using the trained learner on test data
                llf_task = learner.predict(
                    test_x[((which_task - 1) * 100) : (which_task * 100), :],
                    task_id=task_num,
                )
                acc = np.mean(
                    llf_task == test_y[((which_task - 1) * 100) : (which_task * 100)]
                )
        ...
        acc = np.mean(
            predictions == test_y[((which_task - 1) * 100) : (which_task * 100)]
        )
        accuracies_across_tasks.append(acc)
