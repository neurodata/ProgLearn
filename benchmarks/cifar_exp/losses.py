...
def supervised_contrastive_loss(yTrue, yPred):
  ...
  negative_sum = tf.math.log(tf.reduce_sum(y * tf.exp(r_dists), axis=1))  # y zeros diagonal 1's
  positive_sum = (1 - y) * r_dists

  n_nonzero = tf.math.reduce_sum(1-y, axis=1) - 1  # Subtract diagonal
  positive_sum = tf.reduce_sum(positive_sum, axis=1) / tf.cast(n_nonzero, positive_sum.dtype)
