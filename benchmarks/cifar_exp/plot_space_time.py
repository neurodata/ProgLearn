...
for alg in range(total_alg): 
    ...
    for slot in range(slots):
        for shift in range(shifts):
            if alg < 2:
                filename = './result/mem_res/'+model_file[alg]+'_'+str(shift+1)+'_'+str(slot)+'.pickle'
            elif alg == 2 or alg == 3:
                filename = './result/mem_res/'+model_file[alg]+str(shift+1)+'_'+str(slot)+'.pickle'
            else:
                filename = './result/mem_res/'+model_file[alg]+'-'+str(shift+1)+'-'+str(slot+1)+'.pkl'
