...
for shift in range(shifts):
    filename = 'result/result/'+model+str(ntrees)+'_'+str(shift+1)+'_'+algo1_name+'.pickle'
    filename2 = 'result/result/'+model+str(ntrees)+'_'+str(shift+1)+'_'+algo2_name+'.pickle'
    ...
    for ii in range(10):
        ...
        err2[ii].extend(
         1 - np.array(
             multitask_df2[multitask_df2['base_task']==ii+1]['accuracy']
         )
        )
        multitask_inference_times[ii].extend(np.array(multitask_df[multitask_df['base_task']==ii+1]['multitask_inference_times']))
        multitask_inference_times2[ii].extend(np.array(multitask_df2[multitask_df2['base_task']==ii+1]['multitask_inference_times']))
    ...
    single_task_inference_time_tmp2[count].extend(np.array(single_task_df2['single_task_inference_times']))
...
multitask_inference_time2 = [np.mean(multitask_inference_time2[i]) for i in range(len(multitask_inference_time2))]
...
ax[0][0].plot(np.arange(1,n_tasks+1), difference, c='red', marker='.', markersize=14, linewidth=3)
