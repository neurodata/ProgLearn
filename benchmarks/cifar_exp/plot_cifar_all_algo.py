...
#%%
te = {
    "L2N": np.zeros(10, dtype=float),
    "L2F": np.zeros(10, dtype=float),
    "Prog-NN": np.zeros(10, dtype=float),
    "DF-CNN": np.zeros(10, dtype=float),
    "LwF": np.zeros(10, dtype=float),
    "EWC": np.zeros(10, dtype=float),
    "Online EWC": np.zeros(10, dtype=float),
    "SI": np.zeros(10, dtype=float),
}
...
mean_te = {
    ...
    "DF-CNN": [np.mean(te["DF-CNN"])],
    "LwF": [np.mean(te["LwF"])],
    "EWC": [np.mean(te["EWC"])],
    "Online EWC": [np.mean(te["Online EWC"])],
    "SI": [np.mean(te["SI"])],
}
...
for i, fte in enumerate(ftes):
    ...
    ax[0][0].plot(
        np.arange(1, 11), fte, c=clr[i], marker=".", markersize=12, label=alg_name[i]
    )
    ...
    for j in range(0, total_alg):
        if j == 0:
            if i == 0:
                ...
                ax[0][1].plot(
                    ns, et[j, :], marker=".", markersize=8, c=c[j], linewidth=3
                )
        elif j == 1:
            if i == 0:
                ax[0][1].plot(
            ...
            if i == 0:
                ax[0][1].plot(
                    ns, et[j, :], marker=".", markersize=8, label=alg_name[j], c=c[j]
                )
            else:
...
ax[0][1].set_yticks([0.4, 0.6, 0.8, 0.9, 1, 1.1, 1.2])
...
ax[1][0].set_yticks([0.4, 0.6, 0.8, 0.9, 1, 1.1, 1.2])
...
ax_ = sns.stripplot(
    x="Algorithms", y="Transfer Efficieny", data=df, palette=c, size=6, ax=ax[1][1]
)
...
ax_.set_yticks([0.4, 0.6, 0.8, 1, 1.2, 1.4])
