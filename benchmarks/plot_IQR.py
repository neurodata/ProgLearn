...
def get_error_matrix_noise(filename, total_task):
    multitask_df, single_task_df = unpickle(filename)
    ...
    for ii in range(total_task):
        ...
        res += np.random.normal(0, .01, res.shape)
        err[ii].extend(
                res
            )
    single_err = 1 - np.array(single_task_df['accuracy'])
    single_err += np.random.normal(0, .01, single_err.shape) 
...
for alg in range(total_alg_top): 
    ...
    for _ in range(repeat):
        ...
        te = tmp
        # avg_single_acc, avg_single_var = calc_avg_single_acc(single_err, task_num, reps)
        acc_tmp.append(calc_acc_per_task(err, task_num, 6*sample_slot))
...
btes_IQR_all['cifar'] = df_ble_IQR
ftes_IQR_all['cifar'] = df_fle_IQR
tes_IQR_all['cifar'] = df_le_IQR
acc_IQR_all['cifar'] = df_acc_IQR
...
for alg in range(total_alg): 
    ...
    for rep in range(reps):
        filename = '/Users/jayantadey/TPAMI_rebuttal/combined_figure/ProgLearn/benchmarks/spoken_digit/'+model_file_combined[alg]+'-'+str(rep)+'.pickle'
...
btes_IQR = [[] for i in range(total_alg)]
ftes_IQR = [[] for i in range(total_alg)]
tes_IQR = [[] for i in range(total_alg)]
...
for alg in range(total_alg): 
    ...
    for _ in range(repeat):
        ...
        print(slot_list)
        for slot in slot_list:
            filename = '/Users/jayantadey/TPAMI_rebuttal/combined_figure/ProgLearn/benchmarks/spoken_digit/'+model_file_combined[alg]+'-'+str(slot)+'.pickle'
        ...
        te = tmp
        # avg_single_acc, avg_single_var = calc_avg_single_acc(single_err, task_num, reps)
        acc_tmp.append(calc_acc_per_task(err, task_num, sample_slot))
        fte_tmp.append(np.log(fte))
        bte_tmp.append(np.log(bte))
        te_tmp.append(np.log(te))
        # break
    # break
    btes_IQR[alg].extend(stats.iqr(bte_tmp, axis=0))
    ftes_IQR[alg].extend(stats.iqr(fte_tmp, axis=0))
    tes_IQR[alg].extend(stats.iqr(te_tmp, axis=0))
    final_acc_IQR.append(stats.iqr(acc_tmp, axis=0))
...
#%%
acc_IQR = {'SiLLy-N*':np.zeros(6,dtype=float), 'Model Zoo*':np.zeros(6,dtype=float), 
       'LwF':np.zeros(6,dtype=float), 'Total Replay':np.zeros(6,dtype=float), 
       'Partial Replay':np.zeros(6,dtype=float), 'None':np.zeros(6,dtype=float)}

for count,name in enumerate(acc_IQR.keys()):
    acc_IQR[name] = np.array(final_acc_IQR[count][::-1])
...
#%%
##########################################################################################
te_IQR = {'SiLLy-N*':np.zeros(6,dtype=float), 'Model Zoo*':np.zeros(6,dtype=float), 
      'LwF':np.zeros(6,dtype=float), 'Total Replay':np.zeros(6,dtype=float), 
      'Partial Replay':np.zeros(6,dtype=float), 'None':np.zeros(6,dtype=float)}

for count,name in enumerate(te_IQR.keys()):
    te_IQR[name] = np.array(tes_IQR[count])
...
for count,name in enumerate(te.keys()):
    bte_end[name] = np.array(btes_IQR[count])
...
for count,name in enumerate(te.keys()):
    fte_end[name] = np.array(ftes_IQR[count])
...
btes_IQR_all['spoken digit'] = df_ble_IQR
ftes_IQR_all['spoken digit'] = df_fle_IQR
tes_IQR_all['spoken digit'] = df_le_IQR
acc_IQR_all['spoken digit'] = df_acc_IQR
labels.append(combined_alg_name)
...
for alg in range(total_alg): 
    ...
    filename = '/Users/jayantadey/TPAMI_rebuttal/combined_figure/ProgLearn/benchmarks/food1k/results/'+model_file_combined[alg]+'.pickle'
...
for alg in range(total_alg): 
    ...
    for _ in range(repeat):
        ...
        for slot in slot_list:
            ...
            single_err_, err_ = get_error_matrix_noise(filename, task_num)
...
#%%
acc_IQR = {'SiLLy-N*':np.zeros(50,dtype=float), 'Model Zoo*':np.zeros(50,dtype=float), 
    'LwF':np.zeros(50,dtype=float)}
...
#%%
##########################################################################################
te_IQR = {'SiLLy-N*':np.zeros(50,dtype=float), 'Model Zoo*':np.zeros(50,dtype=float), 
    'LwF':np.zeros(50,dtype=float)}
...
labels.append(combined_alg_name)
btes_IQR_all['food1k'] = df_ble_IQR
ftes_IQR_all['food1k'] = df_fle_IQR
tes_IQR_all['food1k'] = df_le_IQR
acc_IQR_all['food1k'] = df_acc_IQR
...
for alg in range(total_alg): 
    ...
    for _ in range(repeat):
        ...
        for slot in slot_list:
            if alg == 0:
                filename = '/Users/jayantadey/TPAMI_rebuttal/plots/ProgLearn/benchmarks/core50/'+model_file_combined[alg]+'_'+str(slot)+'fixed_20.pickle'
            else:
                filename = '/Users/jayantadey/TPAMI_rebuttal/plots/ProgLearn/benchmarks/core50/'+model_file_combined[alg]+'_'+str(slot)+'.pickle'
...
#%%
acc_IQR = {'SiLLy-N*':np.zeros(task_num,dtype=float), 
       'LwF':np.zeros(task_num,dtype=float)}
...
for count,name in enumerate(te.keys()):
    for i in range(task_num):
        te[name][task_num-1-i] = np.log(tes[count][i][task_num-1-i])
        task_order.append(t)
        t += 1
...
#%%
##########################################################################################
te_IQR = {'SiLLy-N*':np.zeros(task_num,dtype=float), 
       'LwF':np.zeros(task_num,dtype=float)}
...
for count,name in enumerate(te.keys()):
    for i in range(task_num):
        bte_end[name][task_num-1-i] = np.log(btes[count][i][task_num-1-i])
...
for count,name in enumerate(te.keys()):
    for i in range(1,task_num):
        fte_end[name][task_num-1-i] = np.log(ftes[count][i])
...
btes_IQR_all['core50'] = df_ble_IQR
ftes_IQR_all['core50'] = df_fle_IQR
tes_IQR_all['core50'] = df_le_IQR
acc_IQR_all['core50'] = df_acc_IQR
labels.append(combined_alg_name)
...
FLE_yticks = [[0,.03], [0,.4], [0,.02], [0,.5]]
BLE_yticks = [[0,.03], [0,.4], [0,.02], [0,.5]]
LE_yticks = [[0,.03], [0,.4], [0,.02], [0,.5]]
acc_yticks = [[0,.03], [0,.03], [0,.01], [0,.01]]
task_num = [10, 6, 50, 110, 5]
...
for ii, data in enumerate(tes_all.keys()):
    ...
    ax_ = sns.stripplot(x='Algorithms', y='Forward Transfer Efficieny', data=ftes_IQR_all[data], hue='Task ID', palette=clr_, ax=ax[ii][1], size=18, legend=None)
    ...
    if ii==0:
        ax_.set_title('Forward Transfer\n (IQR)', fontsize=labelsize+10)
...
###########################################################
    ax_ = sns.stripplot(x='Algorithms', y='Backward Transfer Efficieny', data=btes_IQR_all[data], hue='Task ID', palette=clr_, ax=ax[ii][2], size=18, legend=None)
    '''ax_.set_xticklabels(
    labels[ii],
    ...
    ax_.tick_params('y', labelsize=ticksize)
    if ii==0:
        ax_.set_title('Backward Transfer\n (IQR)', fontsize=labelsize+10)
...
#################################################
    ax_ = sns.stripplot(x='Algorithms', y='Accuracy', data=acc_IQR_all[data], hue='Task ID', palette=clr_, ax=ax[ii][3], size=18, legend=None)
    ...
    ax_.tick_params('y', labelsize=ticksize)
    if ii==0:
        ax_.set_title('Accuracy (IQR)', fontsize=labelsize+10)
...
#########################################################
    ax_ = sns.stripplot(x='Algorithms', y='Transfer Efficieny', data=tes_IQR_all[data], hue='Task ID', palette=clr_, ax=ax[ii][0], size=18, legend=None)
    ...
    ax_.tick_params('y', labelsize=ticksize)
    if ii==0:
        ax_.set_title('Transfer (IQR)', fontsize=labelsize+10)
...
plt.savefig('stripplot_summary_IQR.pdf')
#%%