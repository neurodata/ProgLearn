...
def LF_experiment(data_x, data_y, angle, model, granularity, reps=1, ntrees=29, acorn=None):
    ...
    for rep in range(reps):
        ...
        with tf.device('/gpu:'+str(int(angle //  granularity) % 4)):
            ...
            progressive_learner.add_task(tmp_data1, train_y1, num_transformers=ntrees, decider_kwargs = {"classes" : np.unique(train_y1)})
            progressive_learner.add_transformer(tmp_data2, train_y2, num_transformers=ntrees, backward_task_ids=[0])
...
if model == "dnn":
    ...
    Parallel(n_jobs=-1, verbose=1)(delayed(LF_experiment)(data_x, data_y, angle, model, granularity, reps=5, ntrees=16, acorn=1) for angle in angles)